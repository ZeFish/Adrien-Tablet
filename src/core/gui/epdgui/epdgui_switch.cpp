...
void EPDGUI_Switch::Draw(M5EPD_Canvas* canvas, int16_t dx, int16_t dy) {
    if (_ishide) {
        return;
    }

    if (_event == EVENT_PRESSED) {
        _canvas_pressed->pushToCanvas(_x + dx, _y + dy, canvas);
        return;
    }

    this->_canvas[_state]->pushToCanvas(_x + dx, _y + dy, canvas);
}
