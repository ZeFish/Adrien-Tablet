...
void EPDGUI_Button::Draw(M5EPD_Canvas* canvas, int16_t dx, int16_t dy) {
    if (_ishide) {
        return;
    }

    if (_state == EVENT_NONE || _state == EVENT_RELEASED) {
        _CanvasNormal->pushToCanvas(_x + dx, _y + dy, canvas);
    } else if (_state == EVENT_PRESSED) {
        _CanvasPressed->pushToCanvas(_x + dx, _y + dy, canvas);
    }
}
