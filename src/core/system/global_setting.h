...
// 5分钟提醒即将关机 (5 minutes before shutdown prompt)
static const uint32_t TIME_BEFORE_SHUTDOWN_PROMPT_MS = 5 * 60 * 1000;
// 显示30秒钟的关机提示 (Show shutdown prompt for 30 seconds)
...
void Shutdown();
void Shutdown();
bool DrawCustomWallpaper(M5EPD_Canvas *canvas, const char *path, int x = 0, int y = 0);
void SetCustomWallpaperPath(String path);
extern String global_custom_wallpaper_path;
