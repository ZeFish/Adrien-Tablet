...
public:
    HomeControlApp() : AppBase("Home Control") {}
    ...
    int init(epdgui_args_vector_t &args) override {
        ...
        if (_tile1) _tile1->attach();
        if (_tile2) _tile2->attach();
        if (_tile3) _tile3->attach();
        if (_tile4) _tile4->attach();
        if (_tile5) _tile5->attach();
        if (_tile6) _tile6->attach();
...
private:
    ...
    void InitComponents() {
        ...
        exitbtn("Home");
        _canvas_title->drawString("Home Control", 270, 34);
