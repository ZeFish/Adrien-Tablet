...
#include "apps/Settings/frame_setting_wallpaper.h"
#include "WiFi.h"
...
void key_synctime_cb(epdgui_args_vector_t &args) {
    ...
    // M5EPD_Canvas *tzone = (M5EPD_Canvas *)(args[0]); // Argument 0 is now timezone canvas if needed
...
Frame_Setting::Frame_Setting(void) : Frame_Base() {
    ...
    _key_syncntp->AddArgs(EPDGUI_Button::EVENT_RELEASED, 0, _timezone_canvas);
...
int Frame_Setting::init(M5EPD_Canvas* canvas, epdgui_args_vector_t &args) {
    _is_run = 1;
